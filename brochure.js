...
const web = async () => {
    ...
    //         switch (i) {
    //             case 0:
    //                 await changeTwoOptions(page, form, 0, ciclo, 4, 1)
    //                 await changeOptions(page, form, 5)
    //                 // const menuBtn = await openList(form, 5)
    //                 // await menuBtn[0].click()
    //                 // await page.waitForTimeout(5000)
    ...
    //             default:
    //                 await changeOneOptions(page, form, 5, i)
    //                 await changeOptions(page, form, 4)
    //                 fs.appendFileSync(`list.txt`, '\n\n\n') 
    //             break
    //         }
    //     }
    ...
    for (let i = 8; i <= 8; i++){
        const ciclo = i
        for (let i = 1; i <= 1; i++){
            await changeTwoOptions(page, form, 0, ciclo, 4, i)
            await changeOptions(page, form, 5)
            fs.appendFileSync(`list.txt`, '\n\n\n')
        }
    }
