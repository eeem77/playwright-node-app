...
export const getIdProducts = async (page) => {
  await page.goto("https://www.apprinting.com/en/seating-charts/products/", {
    timeout: 300000,
  });
  const products = await page.$$eval(".product-box", (node) =>
    node.map((n) => n.className)
...
export const getXmlProducts = async (page) => {
  for await (const id of idProducts) {
    ...
    fs.appendFileSync("list-test-gate-fold-4-25-2025.xml", `${item}\n`);
    //}
    // console.log(`${id} ---> Working`);
...
export const getPricesProducts = async (page) => {
  for await (const id of idProducts) {
    ...
    let totalSumPrice = 0;
    let pricesModel = [];
    fs.appendFileSync("list-audit-prices-is-seating-charts.txt", `${id},`);
    for await (const element of qtyFromInputs) {
      // const inputValue = await element.inputValue();
    ...
    for await (const element of optionsValue) {
      try {
        ...
        fs.appendFileSync(
          "list-audit-prices-is-seating-charts.txt",
    ...
    fs.appendFileSync("list-audit-prices-is-seating-charts.txt", `${report}\n`);
...
export const getAttributes = async (page) => {
  for await (const id of idProducts) {
    ...
    for await (const block of blocks) {
      ...
      fs.appendFileSync("list-audit-prices-is-seating-charts.txt", `${attributesLen},`);
    }
    fs.appendFileSync("list-audit-prices-is-seating-charts.txt", `\n`);
  }
};
