...
export const getPricesProducts = async (page) => {
  for await (const id of idProducts) {
    ...
    let totalSumPrice = 0;
    let pricesModel = [];
    fs.appendFileSync("list-audit-prices-fancy-luxury.txt", `${id},`);
    for await (const element of qtyFromInputs) {
      // const inputValue = await element.inputValue();
    ...
    for await (const element of optionsValue) {
      try {
        ...
        fs.appendFileSync(
          "list-audit-prices-fancy-luxury.txt",
    ...
    fs.appendFileSync("list-audit-prices-fancy-luxury.txt", `${report}\n`);
...
export const getAttributes = async (page) => {
  for await (const id of idProducts) {
    ...
    for await (const block of blocks) {
      ...
      fs.appendFileSync("list-audit-prices-fancy-luxury.txt", `${attributesLen},`);
    }
    fs.appendFileSync("list-audit-prices-fancy-luxury.txt", `\n`);
  }
};
