...
export const getXmlProducts = async (page) => {
  for await (const id of idProducts) {
    ...
    fs.appendFileSync("list-test-welcome-sign-4-10-2025.xml", `${item}\n`);
    //}
    // console.log(`${id} ---> Working`);
...
export const getPricesProducts = async (page) => {
  for await (const id of idProducts) {
    ...
    let totalSumPrice = 0;
    let pricesModel = [];
    fs.appendFileSync("list-audit-prices-6-1-4-square-denali.txt", `${id},`);
    for await (const element of qtyFromInputs) {
      // const inputValue = await element.inputValue();
    ...
    for await (const element of optionsValue) {
      try {
        ...
        fs.appendFileSync(
          "list-audit-prices-6-1-4-square-denali.txt",
    ...
    fs.appendFileSync("list-audit-prices-6-1-4-square-denali.txt", `${report}\n`);
...
export const getAttributes = async (page) => {
  for await (const id of idProducts) {
    ...
    for await (const block of blocks) {
      ...
      fs.appendFileSync("list-audit-prices-6-1-4-square-denali.txt", `${attributesLen},`);
    }
    fs.appendFileSync("list-audit-prices-6-1-4-square-denali.txt", `\n`);
  }
};
