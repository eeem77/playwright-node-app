...
export const getPricesProducts = async (page) => {
  for await (const id of idProducts) {
    ...
    let totalSumPrice = 0;
    let pricesModel = [];
    fs.appendFileSync("list-audit-prices-spanish-english.txt", `${id},`);
    for await (const element of qtyFromInputs) {
      // const inputValue = await element.inputValue();
    ...
    for await (const element of optionsValue) {
      try {
        ...
        fs.appendFileSync(
          "list-audit-prices-spanish-english.txt",
    ...
    fs.appendFileSync("list-audit-prices-spanish-english.txt", `${report}\n`);
...
export const getAttributes = async (page) => {
  for await (const id of idProducts) {
    ...
    for await (const block of blocks) {
      ...
      fs.appendFileSync("list-audit-prices-spanish-english.txt", `${attributesLen},`);
    }
    fs.appendFileSync("list-audit-prices-spanish-english.txt", `\n`);
  }
};
