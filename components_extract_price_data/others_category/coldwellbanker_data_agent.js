...
const web = async () => {
  ...
  await page.goto(
    "https://www.coldwellbanker.com/city/ca/santa-clara/agents?page=1"
  );
  //await page.waitForTimeout(5000);
  await page.waitForURL('https://www.coldwellbanker.com/city/ca/santa-clara/agents?page=1');




  const nameAgent = await page.$$('[data-testid="office-name"]');
  const licenseNumberAgent = await page.$$('[data-testid="license-number"]');

  console.log(nameAgent.length, licenseNumberAgent.length);
  ...
  //   if (i == 0){
  //     await page.goto(
  //       `https://www.sanjose.com/restaurants/business-directory/`
  //     );  
  //     await page.waitForURL(`https://www.sanjose.com/restaurants/business-directory/`);
  //   } else {
  //     await page.goto(
  //       `https://www.sanjose.com/restaurants/business-directory/${i}`
  //     );  
  //     await page.waitForURL(`https://www.sanjose.com/restaurants/business-directory/${i}`);
  //   }

  //   const restaurantGroup = await page.$$('.listing-meta');
  //   var resturantData = [];

  //   for await (const element of restaurantGroup) {
  //     const restaurantNameElement = await element.$('h2');
  //     const restaurantLink = await restaurantNameElement.$eval('a', node => node.getAttribute('href'));
  //     const clickweb = await restaurantNameElement.$('a')
  //     const restaurantName = await element.$eval('h2', node => node.innerText);
  //     const restaurantLocation = await element.$eval('.location', node => node.innerText);
  //     const restaurantPhone = await element.$eval('.tel', node => node.innerText);

  //     await page.goto(
  //       `https://www.sanjose.com${restaurantLink}`
  //     );  
  //     await page.waitForURL(`https://www.sanjose.com${restaurantLink}`);

  //     const infoVitals = await page.$('#listingVitals');
  //     const infoVitalsLinks = await infoVitals.$$('a');
  //     const web = await infoVitalsLinks[1].getAttribute('href');

  //     resturantData = [...[restaurantName+'|', restaurantLocation+'|', restaurantPhone+'|', restaurantLink+'|', web]];
  //     fs.appendFileSync(`list.txt`, resturantData.toString() + "\n");
  ...
  //     // } else {
  //     //   await page.goto(
  //     //     `https://www.sanjose.com/restaurants/business-directory/${number}`
  //     //   );  
  //     //   await page.waitForURL(`https://www.sanjose.com/restaurants/business-directory/${number}`);
  //     // }
  //     // console.log('hola');
  //     //const restaurantBlurb = await element.$eval('.blurb', node => node.innerText);
