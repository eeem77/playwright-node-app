...
const web = async () => {
  ...
  const actionOptions = async (size) => {
    await changeOption(page, "#attribute211", size);
  ...
  for await (const size of sizes) {
    ...
    await actionOptions(size);
    labelArray = [];
    
    // AUTOSELECT COATING
    // const selectCoating = await page.$("#attribute199");
    // const optionsCoating = await selectCoating.$$eval("option", (node) =>
    //   node.map((n) => n.innerText)
    // );
    // console.log(optionsCoating);

    // AUTOSELECT FOLDING
    const selectFolding = await page.$("#attribute205");
    const optionsFolding = await selectFolding.$$eval("option", (node) =>
      node.map((n) => n.innerText)
    );
    console.log(optionsFolding);

    for await (const el of optionsFolding) {
      if (el !== "Choose an Option...") {

        await actionOptions(size);

        await changeOption(page, "#attribute205", el);
        await page.waitForTimeout(2000);
        ...
        // } else if(size === '4.25" x 11"' && el === "UV on 4-color side"){
        //   await changeOption(page, "#attribute205", "Roll Fold");
        // } else {
