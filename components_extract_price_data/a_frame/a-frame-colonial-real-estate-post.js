...
const changeOption = async (page, id, label) => {
  ...
  await option.selectOption({ label })
...
const web = async () => {
  ...
  await page.goto('https://4over.com/real-estate-post', { timeout: 300000 })
  ...
  for (let i = 1; i < 51; i++) {
    ...
    //     //%
    const qty = await page.$('.runsize-qty-cell')
    await qty.fill(i)
    // }
    ...
    for (let i = 0; i < runsize.length; i++) {
      const nodo = labelArray
      const data = [...nodo, runsize[i], markupPriceClean[i].replace(',', '')]
