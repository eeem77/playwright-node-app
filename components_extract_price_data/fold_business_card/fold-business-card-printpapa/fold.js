...
const web = async () => {
  ...
  await page.goto(
    "https://www.printpapa.com/eshop/pc/Fold-Over-Business-Cards-3-5x4-3-5x2-588p2665.htm",
    {
      timeout: 300000,
    }
  );
  ...
  for await (const size of optionsProductSize) {
    ...
    // PAPER
    // const productPaper = await page.$("#CAG5");
    ...
    // UV Coating
    // const productCoating = await page.$("#CAG231");
    // const optionsProductCoating = await productCoating.$$eval("option", (node) =>
    //   node.map((n) => n.value)
    // );
    // // const optionsProductFoldFinal = optionsProductFold.filter(
    // //   (option) => option !== "5736_-0.012_0_0_0_0_0"
    // // );
    // console.log(optionsProductCoating);

    // Turnaround Time
    const productTime = await page.$("#CAG15");
    const optionsProductTime = await productTime.$$eval("option", (node) =>
      node.map((n) => n.value)
    );
    // const optionsProductFoldFinal = optionsProductFold.filter(
    //   (option) => option !== "5736_-0.012_0_0_0_0_0"
    // );
    console.log(optionsProductTime);

    for await (const element of optionsProductTime) {
      await productTime.selectOption(element);
      await page.waitForTimeout(2000);
